...
# actions = []
# data = ActionDataset('train')
# for i in range(len(data)):
# 	obs, action = data[i]
# 	actions.append(action)
# stack = np.hstack(actions)
# print(stack.shape())
...
train_class_loss_weights = np.array([0.197, 0.047, 0.44, 0.211, 0.031, 0.074])
...
def train(max_iter, batch_size=16, log_dir=None):
	...
	for t in range(max_iter):
		...
		batch_obs = batch_obs.float().permute(0,1,4,2,3)
		batch_actions = batch_actions.float().permute(0,2,1)
